...
use crate::state::{Config, ConfigMsg, CONFIG, CONFIG_MSG};
...
pub fn instantiate(
    ...
    let config_msg = ConfigMsg {
        last_received_message: "hihihihihihi".to_string(),
    };
...
pub fn query(deps: Deps, _env: Env, msg: QueryMsg) -> StdResult<Binary> {
    match msg {
        QueryMsg::GetConfigMsg {} => to_binary(&CONFIG_MSG.load(deps.storage)?),
    }
}
